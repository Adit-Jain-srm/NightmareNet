...
def main():
    ...
    try:
        ...
        if args.dry_run:
            ...
            logger.info("Config summary: train_model=%s, eval_model=%s", train_model_name, eval_model_name)
            return
        ...
        train_dataloader_small = _tokenize_dataset(
            dataset_wrapper.train_data, trainer_small.tokenizer, text_column, max_length, batch_size, label_column
        )
        ...
        nightmare_dataloader_small = _tokenize_dataset(
            nightmare_data, trainer_small.tokenizer, text_column, max_length, batch_size, label_column
        )
        ...
        clean_dataloader_small = _tokenize_dataset(
            dataset_wrapper.test_data, trainer_small.tokenizer, text_column, max_length, batch_size, label_column
        )
        ...
        model_large = AutoModelForSequenceClassification.from_pretrained(eval_model_name, num_labels=num_labels).to(device)

        clean_dataloader_large = _tokenize_dataset(
            dataset_wrapper.test_data, tokenizer_large, text_column, max_length, batch_size, label_column
        )
        ...
        if args.finetune_large:
            ...
            train_dataloader_large = _tokenize_dataset(
                dataset_wrapper.train_data, trainer_large.tokenizer, text_column, max_length, batch_size, label_column
            )
            dream_dataloader_large = _tokenize_dataset(
                dream_data_large, trainer_large.tokenizer, text_column, max_length, batch_size, label_column
            )
            nightmare_dataloader_large = _tokenize_dataset(
                nightmare_data_large, trainer_large.tokenizer, text_column, max_length, batch_size, label_column
            )
            ...
            inference_time_large_finetuned = (time.time() - start_time) / len(dataset_wrapper.test_data)
        ...
        summary = {
            ...
            "BERT-large (zero-shot)": extract_metrics(results_large_zeroshot, inference_time_large_zeroshot),
        }
        
        if results_large_finetuned:
            summary["BERT-large + NightmareNet"] = extract_metrics(results_large_finetuned, inference_time_large_finetuned)
